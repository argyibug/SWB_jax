...
from bogoliubov_transform import Bogoliubov_transform_2, Bogoliubov_constraint, saddle_point_sum
...
class SaddlePointOptimizer:
    ...
    def saddle_point_eq_number(self, lambda_param: float, A1: complex, B1: complex) -> float:
        ...
        # res_eq=Bogoliubov_constraint(0, self.k1, self.k2, self.Q1, self.Q2, A1, A2, A3, B1, B2, B3, lambda_param, self.h)
        # print("check eq number constraint:", res_eq, lambda_param)
        # print("A1:", A1, "B1:", B1, "lambda:", lambda_param)
        # print("--------------------------------------------------")

        Ubov = Bogoliubov_transform_2(
            0, self.k1, self.k2, self.Q1, self.Q2, A1, A2, A3, B1, B2, B3, lambda_param, self.h)[0]
        # print("--------------------------------------------------")
            
        lam = saddle_point_sum(Ubov, self.k1, self.k2, self.Q1, self.Q2)[0]
        z = (lam - 2.0 * self.S)**2
    ...
    def saddle_point_constraint(self, lambda_param: float, A1: complex, B1: complex) -> List[float]:
        ...
        con_eig = Bogoliubov_constraint(0, self.k1, self.k2, self.Q1, self.Q2, A1, A2, A3, B1, B2, B3, 
                                        lambda_param, self.h)
    ...
    def saddle_point_eq_auxiliary_fields(self, x: np.ndarray) -> float:
        ...
        AA, BB = saddle_point_sum(Ubov, self.k1, self.k2, self.Q1, self.Q2)[1:3]  # 修复：[1:3]返回2个元素
...
def optimize_saddle_point(k1: np.ndarray, k2: np.ndarray, h: float, Q1: float, Q2: float, 
    ...
    # 创建优化器
    optimizer = SaddlePointOptimizer(S=S)
