...
def saddle_point_sum_jax(Ubov: jnp.ndarray, k1: jnp.ndarray, k2: jnp.ndarray, 
    ...
    """
    # 兼容单k点输入: Ubov 可以是 (4, 4) 或 (nk, 4, 4)
    Ubov_batch = Ubov[jnp.newaxis, :, :] if Ubov.ndim == 2 else Ubov
    k1_batch = jnp.atleast_1d(k1)
    nk = Ubov_batch.shape[0]
    ...
    # 向量化计算所有k点
    all_contributions = jax.vmap(compute_single_k_contribution)(Ubov_batch, k1_batch)
