...
def compute_single_k_contribution(H_k: jnp.ndarray, Ubov: jnp.ndarray, ek: jnp.ndarray) -> jnp.ndarray:
    """
    计算单个k点在Bogoliubov基底下的贡献（用于调试/验证）。
    ...
    --------
    jnp.ndarray
        transformed - diag(ek)，理想情况下应接近零矩阵
    """
    Ubov_dag = jnp.conjugate(jnp.transpose(Ubov))
    transformed = Ubov_dag @ H_k @ Ubov
    return transformed - jnp.diag(ek)
