...
def Ham_jax(omega: float, k1: jnp.ndarray, k2: jnp.ndarray,
    ...
    for b in range(bond_n):
        ...
        H = H.at[:, s1_upo_idx, s2_upd_idx].set(Jplus[b] * B[b] * jnp.cos(kr))
        H = H.at[:, s1_dwo_idx, s2_dwd_idx].set(Jplus[b] * B[b] * jnp.cos(kr))
        H = H.at[:, s2_upo_idx, s1_upd_idx].set(Jplus[b] * B[b] * jnp.cos(kr))
