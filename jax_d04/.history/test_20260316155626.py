...
im = ax.imshow(intensity, cmap="viridis", origin="upper")
...
for i in range(H0.shape[0]):
	for j in range(H0.shape[1]):
		...
		if abs(val.imag) > 1e-8:
			label += f"\n{val.imag:+.2f}i"
		text_color = "white" if intensity[i, j] > 0.5 * vmax else "black"
		txt = ax.text(
			j,
			i,
			label,
			...
			fontweight="bold",
			color=text_color,
			bbox={"boxstyle": "round,pad=0.15", "facecolor": "black", "alpha": 0.22, "edgecolor": "none"},
		)
