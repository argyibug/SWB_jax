...
def bogoliubov_single_k(H_k: jnp.ndarray, n_spin: int) -> Tuple[jnp.ndarray, jnp.ndarray]:
    ...
    jax.debug.print("test ut^H ut:\n{}", jnp.round(ut.conj().T @ ut, 4))
    ...
    # jax.debug.print("test g:\n{}", jnp.round(un.conj().T @ ht @ un, 2))
    jax.debug.print("test un^H un:\n{}", jnp.round(un.conj().T @ un, 4))

    jax.debug.print("test ekk_diag:\n{}", jnp.round(Ubov.conj().T @ H_k @ Ubov, 2))
    jax.debug.print("test g:\n{}", jnp.round(Ubov.conj().T @ g @ Ubov, 2))
