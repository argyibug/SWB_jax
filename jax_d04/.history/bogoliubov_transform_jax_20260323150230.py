...
def saddle_point_sum_jax(Ubov: jnp.ndarray, k1: jnp.ndarray, k2: jnp.ndarray,
    ...
    def compute_single_bond_contribution(b, Jplus, cc_cur, ss_cur, lam_cur):
        ...
        cc_cur = H_cur.at[:, s1_upo_idx, (mat_dim -1 - s2_upd_idx)].add(cos_term)
        cc_cur = H_cur.at[:, s1_dwo_idx, (mat_dim -1 - s2_dwd_idx)].add(cos_term)
        cc_cur = H_cur.at[:, s1_upd_idx, (mat_dim -1 - s2_upo_idx)].add(cos_term)
        cc_cur = H_cur.at[:, s1_dwd_idx, (mat_dim -1 - s2_dwo_idx)].add(cos_term)

        ss_cur = H_cur.at[:, s1_upo_idx, (mat_dim -1 - s2_dwo_idx)].add(-sin_term)
        ss_cur = H_cur.at[:, s1_dwo_idx, (mat_dim -1 - s2_upo_idx)].add(sin_term)
        ss_cur = H_cur.at[:, s1_upd_idx, (mat_dim -1 - s2_dwd_idx)].add(sin_term)
        ss_cur = H_cur.at[:, s1_dwd_idx, (mat_dim -1 - s2_upd_idx)].add(-sin_term)
        
        return cc_cur, ss_cur, lam_cur
