...
def saddle_point_sum_jax(Ubov: jnp.ndarray, k1: jnp.ndarray, k2: jnp.ndarray,
    ...
    nk = Ubov.shape[0]
    bond_n= jnp.minimum(n_bond, bond_tab.shape[0])
    ...
    cc = jnp.zeros((nk, mat_dim, mat_dim), dtype=jnp.complex128)
    cc = cc.at[:, :, :].set(0)
        
    ss = jnp.zeros((nk, mat_dim, mat_dim), dtype=jnp.complex128)
    ss = ss.at[:, :, :].set(0)
        
    lam = jnp.zeros((nk, mat_dim, mat_dim), dtype=jnp.complex128)
    lam = lam.at[:, :, :].set(0)

    def compute_single_k_contribution(ut, k1_i, spin_n=n_spin):
        """计算单个k点的贡献"""
        mat_dim = 4 * spin_n
        diag_up = 1
        diag_dw = 1
        ...
        lam = lam.at[:, idx_upd, (mat_dim -1 - idx_upo)].set(diag_up)
        lam = lam.at[:, idx_dwd, (mat_dim -1 - idx_dwo)].set(diag_dw)
        lam = lam.at[:, idx_upo, (mat_dim -1 - idx_upd)].set(diag_up)
        lam = lam.at[:, idx_dwo, (mat_dim -1 - idx_dwd)].set(diag_dw)

        cc_cur, ss_cur = jax.lax.fori_loop(0, n_bond_eff, compute_single_bond_contribution, (Jplus, cc_cur, ss_cur))
        # 计算 ut @ ut^† 
        ut_uth = ut @ jnp.conj(ut).T
        
        # 计算三个组合矩阵
        combined_0 = ut_uth @ lam - lam
    ...
    def compute_single_bond_contribution(b, Jplus, cc_cur, ss_cur):
