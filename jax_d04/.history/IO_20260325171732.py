...
def convert_bond_table_to_jax_array(bond_table) -> np.ndarray:
    """将 bond_table 转换为 JAX 可用的纯数值矩阵。

    输出列格式为 [i, j, site_i, site_j, rij_x, rij_y, bond_type]。
    其中 rij_x, rij_y 使用晶格基矢坐标，避免 object 数组进入 JAX。
    """
    ...
    return np.asarray(numeric_rows, dtype=np.float64)
