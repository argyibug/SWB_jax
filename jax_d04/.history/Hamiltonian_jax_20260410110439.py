...
def Ham_jax(omega: float, k1: jnp.ndarray, k2: jnp.ndarray,
    ...
    def bond_body(b, H_cur):
        ...
        rij_x = bond_tab[b, 4]
        rij_y = bond_tab[b, 5]
        print(f"Processing bond {b}: s1={s1_idx}, s2={s2_idx}, rij=({rij_x}, {rij_y})")
        ...
        cos_term = 0.5 * Jplus[bond_type] * B[bond_type] * jnp.cos(kr)
        sin_term = 0.5 * Jplus[bond_type] * A[bond_type] * jnp.sin(kr)
        #sin_term = 0

        H_cur = H_cur.at[:, s1_upo_idx, (mat_dim -1 - s2_upd_idx)].add(cos_term)
        H_cur = H_cur.at[:, s1_dwo_idx, (mat_dim -1 - s2_dwd_idx)].add(cos_term)
        H_cur = H_cur.at[:, s1_upd_idx, (mat_dim -1 - s2_upo_idx)].add(cos_term)
        H_cur = H_cur.at[:, s1_dwd_idx, (mat_dim -1 - s2_dwo_idx)].add(cos_term)

        H_cur = H_cur.at[:, s1_upo_idx, (mat_dim -1 - s2_dwo_idx)].add(sin_term)
        H_cur = H_cur.at[:, s1_dwo_idx, (mat_dim -1 - s2_upo_idx)].add(-sin_term)
        H_cur = H_cur.at[:, s1_upd_idx, (mat_dim -1 - s2_dwd_idx)].add(-sin_term)
        H_cur = H_cur.at[:, s1_dwd_idx, (mat_dim -1 - s2_upd_idx)].add(sin_term)
        return H_cur
    ...
    H = H.at[:, idx_upd, (mat_dim -1 - idx_upo)].set(diag_up)
    H = H.at[:, idx_dwd, (mat_dim -1 - idx_dwo)].set(diag_dw)
    H = H.at[:, idx_upo, (mat_dim -1 - idx_upd)].set(diag_up)
    H = H.at[:, idx_dwo, (mat_dim -1 - idx_dwd)].set(diag_dw)

    # 添加对称破缺场项
    H = H.at[:, idx_upd, (mat_dim -1 - idx_dwo)].add(hx_term)
    H = H.at[:, idx_dwo, (mat_dim -1 - idx_upd)].add(hx_term)
    H = H.at[:, idx_dwd, (mat_dim -1 - idx_upo)].add(hx_term)
    H = H.at[:, idx_upo, (mat_dim -1 - idx_dwd)].add(hx_term)
