...
def Ham_jax(omega: float, k1: jnp.ndarray, k2: jnp.ndarray,
    ...
    def bond_body(b, H_cur):
        ...
        sin_term = 1j * 0.5 * Jplus[bond_type] * A[bond_type] * jnp.sin(kr)
        ...
        H_cur = H_cur.at[:, s1_upd_idx, s2_dwd_idx].add(-sin_term)
        H_cur = H_cur.at[:, s1_dwd_idx, s2_upd_idx].add(sin_term)
        return H_cur
