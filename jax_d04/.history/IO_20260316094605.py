...
def gen_bond_table(spin_positions: np.ndarray, unit_vectors: np.ndarray) -> Tuple[np.ndarray, int]:
    """
    生成自旋键表。
            ...
            if i > j:
                ...
                if (dist-1)**2 < 1e-5:  # 可以根据实际情况调整距离阈值
                    ...
                    if (site_i == 0 and site_j == 1) or (site_i == 1 and site_j == 0):
                        type = 0
                        bond_table.append((i, j, site_i, site_j, spin_positions[i] - extand_spin_table[j], type))
    ...
    return bond_table, n_bond

def read_bond_table(filepath: str = 'bond.log') -> Tuple[np.ndarray, int]:
    """
    从文件读取自旋键表。
    ...
    tuple
        (bond_table, n_bond)
        - bond_table: np.ndarray, 形状 (n_bond, 6)，包含键信息
        - n_bond: int，键的数量
    """
    if not os.path.exists(filepath):
        raise FileNotFoundError(f"文件不存在: {filepath}")
    ...
    try:
        n_bond = int(lines[0].strip())
    except Exception as e:
        raise ValueError(f"无法解析第一行的键数量: {str(e)}")
    ...
    for line_idx, line in enumerate(lines[1:], start=2):
        ...
        try:
            ...
            if len(bond_info) != 6:
                raise ValueError(f"第 {line_idx} 行的键信息长度不为 6: {bond_info}")
...
def create_bond_table_file(unit_vector_filepath: str = 'unit_vector.in', cellspin_filepath: str = 'cellspin.in'):
    ...
    [bond_table, n_bond] = gen_bond_table(spin_positions=cell_spins, unit_vectors=unit_vectors)
    return bond_table, n_bond
